    ...
    didSet {
      ...
      if isFront && filter.needsDepth {
        if let fallback = FilterLibrary.shared.firstNonDepthFilter() {
          DebugLog.warning(
            "Depth filter '\(filter.name)' selected on front camera -> switching to '\(fallback.name)'"
          )
      ...
      if isRecording, let family = recordingFilterFamily {
        ...
        if filterFamily != family {
          DebugLog.warning(
            "Filter '\(filter.name)' blocked during recording (locked to \(family.rawValue) filters)"
          )
      ...
      camera.applyDepthPolicy(
        needsDepth: filter.needsDepth, reason: "activeFilter changed to \(filter.name)")
