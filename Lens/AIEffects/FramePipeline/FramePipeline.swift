...
final class FramePipeline: ObservableObject {
  ...
  @Published private(set) var isRenderingPaused: Bool = false

  private let pauseLock = NSLock()
  private var pauseReasons: Set<String> = []
  private var pausedFlag: Bool = false
  ...
  var activeFilter: FilterDefinition? = FilterLibrary.shared.filters.first {
    didSet {
      ...
      DispatchQueue.main.async {
        self.isDepthModeActive = filter.needsDepth
      }
  ...
  private init() {
    ...
    gate.isPausedProvider = { [weak self] in
  ...
  func updateDepthModeActive() {
    let newValue = activeFilter?.needsDepth == true
  ...
  func pauseRendering(reason: String) {
    updatePauseState(addReason: reason, removeReason: nil)
  }

  func resumeRendering(reason: String) {
    updatePauseState(addReason: nil, removeReason: reason)
  }

  func resumeAllRendering() {
    pauseLock.lock()
    ...
    gate.resetTiming()
    lastPhaseTime = CACurrentMediaTime()

    DispatchQueue.main.async {
      self.isRenderingPaused = false
    }
  }

  private func renderingPaused() -> Bool {
    pauseLock.lock()
    let value = pausedFlag
    pauseLock.unlock()
    return value
  }

  private func updatePauseState(addReason: String?, removeReason: String?) {
    pauseLock.lock()

    if let addReason {
      pauseReasons.insert(addReason)
    }

    if let removeReason {
      pauseReasons.remove(removeReason)
    }

    let newValue = !pauseReasons.isEmpty
    pausedFlag = newValue
    ...
    DispatchQueue.main.async {
      self.isRenderingPaused = newValue
    }
  }
}
