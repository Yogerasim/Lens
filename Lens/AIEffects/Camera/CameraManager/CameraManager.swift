...
final class CameraManager: NSObject, ObservableObject {
  ...
  private func switchToDevice(type: AVCaptureDevice.DeviceType, position: AVCaptureDevice.Position)
  {
    ...
    let resetLogical =
      position == .front ? 1.0 : zoomController.lensBaseZoom(for: device.deviceType)
...
extension CameraManager {
  ...
  private func zoomContext(currentDevice: AVCaptureDevice? = nil) -> CameraZoomControllerContext {
    CameraZoomControllerContext(
      ...
      currentDeviceMaxZoomFactor: currentDevice?.activeFormat.videoMaxZoomFactor
        ?? sessionController.currentInput?.device.activeFormat.videoMaxZoomFactor ?? maxZoomFactor
    )
  }
  ...
  private func applyDigitalZoomOnly(logical: CGFloat, publishLogical: Bool) {
    guard
      let publishedLogical = zoomController.applyDigitalZoomOnly(
  ...
  private func switchBackLens(
    to newLens: CameraLensKind, targetLogicalZoom logical: CGFloat, reason: String
  ) {
  ...
  func zoomGestureChanged(logicalZoom: CGFloat) {
    ...
    sessionQueue.async {
      let clampedLogical = self.clamp(
        logicalZoom, self.minimumLogicalZoom, self.maxDigitalZoomForUI)
  ...
  func zoomGestureEnded(targetLogicalZoom: CGFloat) {
    ...
    sessionQueue.async {
      let clampedLogical = self.clamp(
        targetLogicalZoom, self.minimumLogicalZoom, self.maxDigitalZoomForUI)
      ...
      if desiredLens != self.currentLensType {
        self.switchBackLens(
          to: desiredLens, targetLogicalZoom: clampedLogical, reason: "gestureEnded")
      } else {
        self.applyDigitalZoomOnly(logical: clampedLogical, publishLogical: true)
      }
    }
